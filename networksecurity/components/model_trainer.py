...
class ModelTrainer:
    ...
    def train_model(self, x_train, y_train, x_test, y_test):
        ...
        # For every model, we have model report along with the score
        model_report: dict = evaluate_models(X_train=x_train, y_train=y_train, X_test=x_test, y_test=y_test, models = models, param = params)
        
        # get the best model score from dict
        # takes all scores from model_report -> sorts them -> sorts them
        best_model_score = max(sorted(model_report.values()))

        # Get the best model name from dict
        # finds index of best score -> uses that index to get model name
        best_model_name = list(model_report.keys())[
            list(model_report.values()).index(best_model_score)
        ]
        best_model = models[best_model_name]            
        y_train_pred = best_model.predict(x_train)          # Model predicts on training data

        classification_train_metric = get_classfication_score(y_true=y_train, y_pred=y_train_pred)

        # Track the Experiments with MLflow for training
        self.track_mlflow(best_model, classification_train_metric)

        y_test_pred = best_model.predict(x_test)
        classification_test_metric = get_classfication_score(y_true=y_test, y_pred=y_test_pred)

        # Track the Experiments with MLflow for testing
